...
class TestSample(TestCase):
    ...
    def test_init_wrong_template(self):
        ...
        with self.assertRaises(IncompetentQiitaDeveloperError):
            qdb.metadata_template.sample_template.Sample(
                "SKB8.640193", qdb.metadata_template.prep_template.PrepTemplate(1)
            )
    ...
    def test_eq_false_type(self):
        ...
        other = qdb.metadata_template.prep_template.PrepSample(
            self.sample_id, qdb.metadata_template.prep_template.PrepTemplate(1)
        )
    ...
    def test_exists_true(self):
        ...
        self.assertTrue(
            qdb.metadata_template.sample_template.Sample.exists(
                self.sample_id, self.sample_template
            )
        )
    ...
    def test_getitem_required(self):
        ...
        self.assertEqual(self.tester["collection_timestamp"], "2011-11-11 13:00:00")
    ...
    def test_columns_restrictions(self):
        """that it returns STC"""
        self.assertEqual(self.sample_template.columns_restrictions, STC)
...
class TestSampleTemplate(TestCase):
    def setUp(self):
        ...
        self.new_study = qdb.study.Study.create(
            qdb.user.User("test@foo.bar"), "Fried Chicken Microbiome %s" % time(), info
        )
    ...
    def test_values(self):
        ...
        self.assertTrue(isinstance(obs, Iterable))
        exp = {
            qdb.metadata_template.sample_template.Sample("1.SKB1.640202", self.tester),
    ...
    def test_clean_validate_template_error_duplicate_cols(self):
        ...
        self.metadata["SAMPLE_TYPE"] = pd.Series(
            ["foo", "bar", "foobar"], index=self.metadata.index
        )
    ...
    def test_clean_validate_template_columns(self):
        ...
        ST = qdb.metadata_template.sample_template.SampleTemplate
        obs = ST._clean_validate_template(metadata, 2, current_columns=STC)
        metadata_dict = {
            "2.Sample1": {
                "physical_specimen_location": "location1",
                "physical_specimen_remaining": "true",
    ...
    def test_clean_validate_template(self):
        ST = qdb.metadata_template.sample_template.SampleTemplate
        obs = ST._clean_validate_template(self.metadata, 2, current_columns=STC)
        metadata_dict = {
            "2.Sample1": {
                "physical_specimen_location": "location1",
                "physical_specimen_remaining": "true",
    ...
    def test_clean_validate_template_no_forbidden_words2(self):
        ...
        self.metadata.rename(columns={"taxon_id": "linkerprimersequence"}, inplace=True)
        with self.assertRaises(qdb.exceptions.QiitaDBColumnError):
            ST._clean_validate_template(self.metadata, 2)
    ...
    def test_identify_forbidden_words_in_column_names(self):
        ...
        self.assertEqual(
            set(results),
            {"qiita_prep_id", "qiita_study_id", "linkerprimersequence", "sampleid"},
        )
    ...
    def test_silent_drop(self):
        ...
        self.assertNotIn(
            "qiitq_prep_id",
            (ST._clean_validate_template(self.metadata, 2)).columns.tolist(),
        )
    ...
    def test_update_category(self):
        ...
        with self.assertRaises(qdb.exceptions.QiitaDBColumnError):
            self.tester.update_category("missing column", {"1.SKM7.640188": "stuff"})

        negtest = self.tester["1.SKM7.640188"]["country"]

        mapping = {"1.SKB1.640202": "1", "1.SKB5.640181": "2", "1.SKD6.640190": "3"}
    ...
    def test_update_numpy(self):
        ...
        ST = qdb.metadata_template.sample_template.SampleTemplate
        metadata_dict = {
            "Sample1": {"bool_col": "true", "date_col": "2015-09-01 00:00:00"},
            "Sample2": {"bool_col": "true", "date_col": "2015-09-01 00:00:00"},
        }
        metadata = pd.DataFrame.from_dict(metadata_dict, orient="index", dtype=str)
        st = npt.assert_warns(
            qdb.exceptions.QiitaDBWarning, ST.create, metadata, self.new_study
        )
    ...
    def test_to_file(self):
        ...
        close(fd)
        st.to_file(
            fp, {"%s.Sample1" % self.new_study.id, "%s.Sample3" % self.new_study.id}
        )
        self._clean_up_files.append(fp)
    ...
    def test_extend_add_duplicate_samples(self):
        ...
        md_ext = pd.DataFrame.from_dict(self.metadata_dict, orient="index", dtype=str)
    ...
    def test_extend_new_columns(self):
        ...
        self.metadata["texture"] = pd.Series(
            ["val1", "val2", "val3"], index=self.metadata.index
        )
        self.metadata["TOT_NITRO"] = pd.Series(
            ["val_1", "val_2", "val_3"], index=self.metadata.index
        )
    ...
    def test_extend_new_samples_and_columns(self):
        ...
        md_ext["TOT_NITRO"] = pd.Series(
            ["val1", "val2", "val3", "val4"], index=md_ext.index
        )
        # Make sure adding duplicate samples raises warning
        npt.assert_warns(qdb.exceptions.QiitaDBWarning, st.extend, md_ext)
        exp_sample_ids = {
    ...
    def test_extend_update(self):
        ...
        npt.assert_warns(qdb.exceptions.QiitaDBWarning, st.extend_and_update, md_ext)
        exp_sample_ids = {
            "%s.Sample1" % st.id,
            "%s.Sample2" % st.id,
            "%s.Sample3" % st.id,
    ...
    def test_to_dataframe(self):
        ...
        self.assertEqual(
            self.tester.ebi_sample_accessions, obs.qiita_ebi_sample_accessions.to_dict()
        )

    def test_check_restrictions(self):
        obs = self.tester.check_restrictions([STC["EBI"]])
    ...
    def test_ebi_sample_accessions(self):
        ...
        exp = {
            "%s.Sample1" % self.new_study.id: None,
    ...
    def test_ebi_sample_accessions_setter(self):
        ...
        exp_acc = {
            "%s.Sample1" % self.new_study.id: "ERS000100",
    ...
    def test_biosample_accessions_setter(self):
        with self.assertRaises(qdb.exceptions.QiitaDBError):
            self.tester.biosample_accessions = {
                "1.SKB8.640193": "SAMEA110000",
        ...
        exp_acc = {
            "%s.Sample1" % self.new_study.id: "SAMEA110000",
    ...
    def test_validate_template_warning_missing_restrictions(self):
        ...
        obs = st.check_restrictions([STC["EBI"]])
    ...
    def test_validate_errors(self):
        ...
        with catch_warnings(record=True) as warn:
            ...
            exp_error = (
                'Sample "%s.Sample2", column "latitude", wrong value '
                '"wrong latitude"' % self.new_study.id
            )
            self.assertIn(exp_error, message)
            exp_error = (
                'Sample "%s.Sample1", column "collection_timestamp", '
                'wrong value "wrong date"' % self.new_study.id
            )
            self.assertIn(exp_error, message)
            exp_error = (
                'Sample "%s.Sample3", column "latitude", '
