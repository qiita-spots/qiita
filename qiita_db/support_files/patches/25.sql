...
CREATE TABLE qiita.sample_template_edit (
	sample_template_id   bigint  NOT NULL,
	change               varchar  NOT NULL,
	timestamp            timestamp DEFAULT current_timestamp NOT NULL,
	CONSTRAINT idx_sample_template_edit PRIMARY KEY ( sample_template_id, change )
 );

CREATE INDEX idx_sample_template_edit_0 ON qiita.sample_template_edit ( sample_template_id );

COMMENT ON TABLE qiita.sample_template_edit IS 'Holds information on edits made to sample templates';

ALTER TABLE qiita.sample_template_edit ADD CONSTRAINT fk_sample_template_edit FOREIGN KEY ( sample_template_id ) REFERENCES qiita.study( study_id );
