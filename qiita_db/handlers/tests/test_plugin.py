...
from qiita_db.handlers.plugin import _get_command, _get_plugin
...
class PluginHandlerTests(OauthTestingBase):
    ...
    def test_get(self):
        ...
        exp = {
            ...
            "publications": [{"DOI": "10.1038/nmeth.f.303", "PubMed": "20383131"}],
...
class CommandListHandlerTests(OauthTestingBase):
    def test_post(self):
        data = {
            ...
            "required_parameters": dumps({"in_data": ['artifact:["FASTA"]', None]}),
        ...
        obs = self.post(
            "/qiita_db/plugins/QIIMEq2/1.9.1/commands/", data=data, headers=self.header
        )
        self.assertEqual(obs.code, 200)
        ...
        data = {
            ...
            "required_parameters": dumps({"in_data": ['artifact:["BIOM"]', None]}),
            "optional_parameters": dumps(
                {
...
class CommandHandlerTests(OauthTestingBase):
    def test_get_command_does_not_exist(self):
        obs = self.get(
            "/qiita_db/plugins/QIIME/1.9.1/commands/UNKNOWN/", headers=self.header
        )
        self.assertEqual(obs.code, 404)

    def test_get_no_header(self):
        obs = self.get("/qiita_db/plugins/QIIMEq2/1.9.1/commands/Split%20libraries/")
        self.assertEqual(obs.code, 400)
    ...
    def test_get(self):
        ...
        exp = {
            ...
            "description": "Demultiplexes and applies quality control to FASTA data",
            "required_parameters": {
                "input_data": ["artifact", ["FASTA", "FASTA_Sanger", "SFF"]]
            },
            "optional_parameters": {
...
class CommandActivateHandlerTests(OauthTestingBase):
    def test_post_command_does_not_exist(self):
        obs = self.post(
            "/qiita_db/plugins/QIIMEq2/1.9.1/commands/UNKNOWN/activate/",
            headers=self.header,
            data={},
        )
        self.assertEqual(obs.code, 404)
    ...
    def test_post_no_header(self):
        obs = self.post(
            "/qiita_db/plugins/QIIMEq2/1.9.1/commands/Split%20libraries/activate/",
            data={},
        )
        self.assertEqual(obs.code, 400)
...
class ReloadPluginAPItestHandlerTests(OauthTestingBase):
    ...
    def test_post(self):
        obs = self.post("/apitest/reload_plugins/", headers=self.header, data={})
        self.assertEqual(obs.code, 200)
