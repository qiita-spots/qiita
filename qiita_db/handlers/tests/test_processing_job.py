...
class UtilTests(TestCase):
    def test_get_job(self):
        ...
        exp = qdb.processing_job.ProcessingJob("6d368e16-2242-4cf8-87b4-a5dc40bb890b")
...
class JobHandlerTests(OauthTestingBase):
    ...
    def test_get(self):
        obs = self.get(
            "/qiita_db/jobs/6d368e16-2242-4cf8-87b4-a5dc40bb890b", headers=self.header
        )
        ...
        exp = {"command": cmd, "parameters": params, "status": "success", "msg": ""}
...
class HeartbeatHandlerTests(OauthTestingBase):
    def test_post_job_does_not_exists(self):
        obs = self.post(
            "/qiita_db/jobs/do-not-exist/heartbeat/", "", headers=self.header
        )
        self.assertEqual(obs.code, 404)
    ...
    def test_post_job_already_finished(self):
        ...
        self.assertEqual(
            obs.reason, "Can't execute heartbeat on job: already completed"
        )
...
class ActiveStepHandlerTests(OauthTestingBase):
    def test_post_no_header(self):
        obs = self.post("/qiita_db/jobs/063e553b-327c-4818-ab4a-adfe58e49860/step/", "")
        self.assertEqual(obs.code, 400)

    def test_post_job_does_not_exists(self):
        obs = self.post("/qiita_db/jobs/do-not-exist/step/", "", headers=self.header)
        self.assertEqual(obs.code, 404)
    ...
    def test_post_non_running_job(self):
        ...
        self.assertEqual(obs.code, 403)
        self.assertEqual(
            obs.reason, "Cannot change the step of a job whose status is not 'running'"
        )
...
class CompleteHandlerTests(OauthTestingBase):
    ...
    def test_post_job_does_not_exists(self):
        obs = self.post(
            "/qiita_db/jobs/do-not-exist/complete/", "", headers=self.header
        )
        self.assertEqual(obs.code, 404)
    ...
    def test_post_job_not_running(self):
        ...
        self.assertEqual(obs.code, 403)
        self.assertEqual(
            obs.body.decode("ascii"), "Can't complete job: not in a running state"
        )
    ...
    def test_post_job_failure(self):
        ...
        obs = self.post(
            "/qiita_db/jobs/%s/complete/" % job.id, payload, headers=self.header
        )
        self.assertEqual(obs.code, 200)
...
class ProcessingJobAPItestHandlerTests(OauthTestingBase):
    def test_post_processing_job(self):
        ...
        obs = self.post("/apitest/processing_job/", headers=self.header, data=data)
        self.assertEqual(obs.code, 200)
    ...
    def test_post_processing_job_status(self):
        ...
        self.assertEqual(qdb.processing_job.ProcessingJob(job_id).status, "running")
