    ...
    def build_files(self, merge_duplicated_sample_ids):
        ...
        with qdb.sql_connection.TRN:
            ...
            if post_processing_cmds:
                biom_files = self._build_biom_tables(grouped_samples,
                        rename_dup_samples, post_processing_cmds=post_processing_cmds)
            else:
                # preserve the legacy path
                biom_files = self._build_biom_tables(grouped_samples, rename_dup_samples)
            ...
            for label, tables in viewitems(grouped_samples):
                ...
                if post_processing_cmds:
                    ...
                    for cmd in post_processing_cmds:
                        ...
                        params = ' '.join(["%s=%s" % (k,v) for k, v in cmd['script_params'].items()])
                        params = "%s %s" % (params, biom_fp)
                        #if environment is successfully activated,
                        #run script with parameters
                        p = Popen(["source activate %s && python %s %s" %
                            (cmd['script_env'], cmd['script_path'], params)], shell=True, stdout=PIPE)
                        p.wait()
                        #consider processing p.communicate, returning it up the
                        #stack, annotating post_processing_cmds, etc.
                        print(p.communicate())

                    biom_files.append((data_type, biom_fp, post_processing_cmds))
                else:
