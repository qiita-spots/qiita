...
class CommandTests(TestCase):
    def setUp(self):
        ...
        self.parameters = {
            ...
            "opt_mchoice_param": ['mchoice:["opt1", "opt2", "opt3"]', ["opt1", "opt2"]],
    ...
    def test_get_commands_by_input_type(self):
        ...
        obs = list(
            qdb.software.Command.get_commands_by_input_type(
                ["FASTQ", "per_sample_FASTQ"]
            )
        )
        self.assertCountEqual(obs, exp)

        obs = list(qdb.software.Command.get_commands_by_input_type(["FASTQ", "SFF"]))
        ...
        obs = list(
            qdb.software.Command.get_commands_by_input_type(
                ["FASTQ"], active_only=False, exclude_analysis=False, prep_type="18S"
            )
        )
    ...
    def test_exists(self):
        self.assertFalse(qdb.software.Command.exists(self.software, "donotexists"))
        self.assertTrue(qdb.software.Command.exists(self.software, "Split libraries"))

    def test_software(self):
        self.assertEqual(qdb.software.Command(1).software, qdb.software.Software(1))
        self.assertEqual(qdb.software.Command(2).software, qdb.software.Software(1))
    ...
    def test_required_parameters(self):
        ...
        self.assertCountEqual(obs["input_data"][1], exp_params["input_data"][1])

        exp_params = {"input_data": ("artifact", ["SFF", "FASTA", "FASTA_Sanger"])}
    ...
    def test_optional_parameters(self):
        ...
        self.assertEqual(qdb.software.Command(1).optional_parameters, exp_params)
        ...
        self.assertEqual(qdb.software.Command(2).optional_parameters, exp_params)
    ...
    def test_default_parameter_sets(self):
        ...
        exp = [qdb.software.DefaultParameters(8), qdb.software.DefaultParameters(9)]
        self.assertEqual(obs, exp)
    ...
    def test_create(self):
        ...
        self.assertEqual(
            obs.merging_scheme,
            {"parameters": [], "outputs": [], "ignore_parent_command": False},
        )
        ...
        validate = qdb.software.Command.create(
            software, "Validate", "Test creating a validate command", parameters
        )
        self.assertEqual(validate.name, "Validate")
        self.assertEqual(validate.description, "Test creating a validate command")
        exp_required = {
            "template": ("prep_template", [None]),
            "analysis": ("analysis", [None]),
            "files": ("string", [None]),
        ...
        exp_optional = {"name": ["string", "dflt_name"], "provenance": ["string", None]}
        ...
        self.assertEqual(obs.optional_parameters, exp_optional)
        self.assertFalse(obs.analysis_only)
        self.assertEqual(obs.naming_order, ["opt_int_param", "opt_choice_param"])
        exp = {
            "parameters": ["opt_choice_param", "opt_int_param"],
            "outputs": ["out1"],
        ...
        old_cmd = [cmd for cmd in self.software.commands if cmd.name == cmd_name][0]
        ...
        old_validate = [
            c
            for c in qdb.software.Software.from_name_and_version(
...
class SoftwareTestsIter(TestCase):
    ...
    def test_iter(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Could not match 'Split libraries' to a resource allocation!"
        ):
            qdb.software.Command(2).resource_allocation
...
class SoftwareTests(TestCase):
    ...
    def test_from_name_and_version(self):
        ...
        obs = qdb.software.Software.from_name_and_version("BIOM type", "2.1.4 - Qiime2")
    ...
    def test_publications(self):
        self.assertEqual(
            qdb.software.Software(1).publications, [["10.1038/nmeth.f.303", "20383131"]]
        )
    ...
    def test_default_workflows(self):
        ...
        self.assertEqual(qdb.software.DefaultWorkflow(1).artifact_type, "FASTQ")
        ...
        obs = list(qdb.software.DefaultWorkflow.iter(True))
        exp = [qdb.software.DefaultWorkflow(2), qdb.software.DefaultWorkflow(3)]
        self.assertEqual(obs, exp)
        ...
        exp = (
            'This accepts html <a href="https://qiita.ucsd.edu">Qiita!</a>'
            "<br/><br/><b>BYE!</b>"
        )
    ...
    def test_type(self):
        self.assertEqual(qdb.software.Software(1).type, "artifact transformation")
    ...
    def test_create(self):
        ...
        self.assertEqual(obs.description, "This is adding a new software for testing")
        ...
        self.assertEqual(obs.version, "0.1.0")
        self.assertEqual(obs.description, "This is adding another software for testing")
        self.assertEqual(obs.commands, [])
        self.assertEqual(obs.publications, [])
...
class DefaultParametersTests(TestCase):
    def test_exists(self):
        ...
        obs = qdb.software.DefaultParameters.exists(
            cmd,
            max_bad_run_length=3,
            min_per_read_length_fraction=0.65,
            sequence_max_n=0,
            rev_comp_barcode=False,
            rev_comp_mapping_barcodes=False,
            rev_comp=False,
...
class ParametersTests(TestCase):
    ...
    def test_eq(self):
        ...
        b = qdb.software.Parameters.from_default_params(
            next(qdb.software.Command(2).default_parameter_sets), {"input_data": 1}
        )
        self.assertFalse(a == b)
    ...
    def test_from_default_params_error_extra_reqd(self):
        with self.assertRaises(qdb.exceptions.QiitaDBError):
            qdb.software.Parameters.from_default_params(
                qdb.software.DefaultParameters(1), {"input_data": 1, "another_one": 2}
            )
...
class DefaultWorkflowNodeTests(TestCase):
    def test_default_parameter(self):
        ...
        self.assertEqual(obs.default_parameter, qdb.software.DefaultParameters(1))

        obs = qdb.software.DefaultWorkflowNode(2)
        self.assertEqual(obs.default_parameter, qdb.software.DefaultParameters(10))
...
class DefaultWorkflowEdgeTests(TestCase):
    def test_connections(self):
        ...
        self.assertEqual(obs, [["demultiplexed", "input_data", "Demultiplexed"]])


class DefaultWorkflowTests(TestCase):
    def test_name(self):
        self.assertEqual(
            qdb.software.DefaultWorkflow(1).name, "FASTQ upstream workflow"
        )
        self.assertEqual(
            qdb.software.DefaultWorkflow(2).name, "FASTA upstream workflow"
        )
        self.assertEqual(
            qdb.software.DefaultWorkflow(3).name, "Per sample FASTQ upstream workflow"
        )

    def test_graph(self):
        obs = qdb.software.DefaultWorkflow(1).graph
        self.assertTrue(isinstance(obs, nx.DiGraph))
        exp = [qdb.software.DefaultWorkflowNode(1), qdb.software.DefaultWorkflowNode(2)]
        self.assertCountEqual(obs.nodes(), exp)
        exp = [
            (
                qdb.software.DefaultWorkflowNode(1),
        ...
        self.assertTrue(isinstance(obs, nx.DiGraph))
        exp = [qdb.software.DefaultWorkflowNode(3), qdb.software.DefaultWorkflowNode(4)]
        self.assertCountEqual(obs.nodes(), exp)
        exp = [
            (
                qdb.software.DefaultWorkflowNode(3),
