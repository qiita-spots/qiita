...
class TestAnalysis(TestCase):
    ...
    def test_build_files_post_processing_cmd(self):
        ...
        for name, biom_fp, archive_artifact_fp in biom_files:
            # assert that biom_fp generated by build_files is in a reasonable
            # location and is of type 'biom' (contains '.biom' in its path.)
            self.assertIn('qiita_db/support_files/test_data/analysis/',
                          biom_fp)
            self.assertIn('.biom', biom_fp)
            self.assertIn('qiita_db/support_files/test_data/analysis/',
                          archive_artifact_fp)
            self.assertIn('.tre', archive_artifact_fp)
