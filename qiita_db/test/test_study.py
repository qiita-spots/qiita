...
class TestStudyPerson(TestCase):
    ...
    def test_create_studyperson(self):
        new = qdb.study.StudyPerson.create(
            "SomeDude", "somedude@foo.bar", "affil", "111 fake street", "111-121-1313"
        )
        nid = new.id
        self.assertEqual(nid, 4)
        with qdb.sql_connection.TRN:
            qdb.sql_connection.TRN.add(
                "SELECT * FROM qiita.study_person WHERE study_person_id = %d" % nid
            )
    ...
    def test_retrieve_non_existant_people(self):
        with self.assertRaises(qdb.exceptions.QiitaDBLookupError):
            qdb.study.StudyPerson.from_name_and_affiliation("Boaty McBoatFace", "UCSD")

        p = qdb.study.StudyPerson.from_name_and_affiliation("LabDude", "knight lab")
    ...
    def test_exists(self):
        ...
        self.assertFalse(qdb.study.StudyPerson.exists("AnotherDude", "knight lab"))
        self.assertFalse(qdb.study.StudyPerson.exists("LabDude", "Another lab"))

    def test_create_studyperson_already_exists(self):
        obs = qdb.study.StudyPerson.create("LabDude", "lab_dude@foo.bar", "knight lab")
...
class TestStudy(TestCase):
    ...
    def test_has_access_public(self):
        ...
        qiita_config.portal = "QIITA"
        self.assertTrue(self.study.has_access(qdb.user.User("demo@microbio.me")))
        qiita_config.portal = "EMP"
        with self.assertRaises(qdb.exceptions.QiitaDBError):
    ...
    def test_has_access_no_public(self):
        self._change_processed_data_status("public")
        self.assertFalse(self.study.has_access(qdb.user.User("demo@microbio.me"), True))
    ...
    def test_can_edit(self):
        ...
        self.assertFalse(self.study.can_edit(qdb.user.User("demo@microbio.me")))
    ...
    def test_share(self):
        ...
        qdb.sql_connection.perform_as_transaction("delete from qiita.study_users")
        ...
        self.assertEqual(self.study.shared_with, [qdb.user.User("shared@foo.bar")])
    ...
    def test_has_access_no_access(self):
        self._change_processed_data_status("sandbox")
        self.assertFalse(self.study.has_access(qdb.user.User("demo@microbio.me")))
    ...
    def test_create_duplicate(self):
        ...
        for tt in to_test:
            with self.assertRaises(qdb.exceptions.QiitaDBDuplicateError):
                qdb.study.Study.create(qdb.user.User("test@foo.bar"), tt, self.info)

    def test_create_study_min_data(self):
        """Insert a study into the database"""
        before = datetime.now()
        obs = qdb.study.Study.create(
            qdb.user.User("test@foo.bar"), "Fried chicken microbiome 1", self.info
        )
        after = datetime.now()
    ...
    def test_create_study_with_investigation(self):
        ...
        with qdb.sql_connection.TRN:
            qdb.sql_connection.TRN.add(
                "SELECT * from qiita.investigation_study WHERE study_id = %s", [new.id]
            )
            obs = qdb.sql_connection.TRN.execute_fetchindex()
        self.assertEqual(obs, [[1, new.id]])

        # testing Study.iter()
        self.assertCountEqual(list(qdb.study.Study.iter()), [qdb.study.Study(1), new])
    ...
    def test_create_study_all_data(self):
        ...
        obs = qdb.study.Study.create(
            qdb.user.User("test@foo.bar"), "Fried chicken microbiome 3", self.info
        )
        ...
        # testing Study.iter()
        self.assertCountEqual(list(qdb.study.Study.iter()), [qdb.study.Study(1), obs])
    ...
    def test_create_missing_required(self):
        ...
        with self.assertRaises(qdb.exceptions.QiitaDBColumnError):
            qdb.study.Study.create(
                qdb.user.User("test@foo.bar"), "Fried Chicken Microbiome 4", self.info
            )
    ...
    def test_create_study_with_not_allowed_key(self):
        ...
        with self.assertRaises(qdb.exceptions.QiitaDBColumnError):
            qdb.study.Study.create(
                qdb.user.User("test@foo.bar"), "Fried Chicken Microbiome 6", self.info
            )
    ...
    def test_create_unknown_db_col(self):
        ...
        with self.assertRaises(qdb.exceptions.QiitaDBColumnError):
            qdb.study.Study.create(
                qdb.user.User("test@foo.bar"), "Fried Chicken Microbiome 7", self.info
            )
    ...
    def test_retrieve_title(self):
        self.assertEqual(
            self.study.title, "Identification of the Microbiomes for Cannabis Soils"
        )
    ...
    def test_ebi_study_accession_setter(self):
        new = qdb.study.Study.create(qdb.user.User("test@foo.bar"), "Test", self.info)
        self.assertEqual(new.ebi_study_accession, None)
    ...
    def test_ebi_submission_status(self):
        ...
        params = qdb.software.Parameters.load(
            cmd, values_dict={"artifact": 2, "submission_type": "ADD"}
        )
        ...
        cmd = qp.get_command("submit_to_EBI")
        params = qdb.software.Parameters.load(
            cmd, values_dict={"artifact": 1, "submission_type": "ADD"}
        )
        job = qdb.processing_job.ProcessingJob.create(user, params, True)
        job._set_error("Killed by Admin")
    ...
    def test_retrieve_investigation(self):
        self.assertEqual(self.study.investigation, qdb.investigation.Investigation(1))
    ...
    def test_retrieve_artifacts(self):
        ...
        self.assertEqual(
            self.study.artifacts(artifact_type="BIOM"),
            [
                qdb.artifact.Artifact(4),
                qdb.artifact.Artifact(5),
                qdb.artifact.Artifact(6),
        ...
        self.assertEqual(
            self.study.artifacts(dtype="18S", artifact_type="BIOM"),
            [qdb.artifact.Artifact(4), qdb.artifact.Artifact(5)],
        )
    ...
    def test_study_tags(self):
        ...
        obs = qdb.study.Study.get_tags()
        exp = {"user": ["I want GOLD!!", "this is my tag"], "admin": ["actual GOLD!"]}
        self.assertEqual(obs, exp)
