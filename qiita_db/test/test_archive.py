...
class ArchiveTest(TestCase):
    def test_insert_from_biom_and_retrieve_feature_values(self):
        ...
        with self.assertRaises(ValueError) as err:
            qdb.archive.Archive.insert_from_artifact(qdb.artifact.Artifact(1), {})
        self.assertEqual(
        ...
        qdb.sql_connection.perform_as_transaction(
            "DELETE FROM qiita.artifact_filepath WHERE artifact_id = %d" % aid
        )
        with self.assertRaises(ValueError) as err:
            qdb.archive.Archive.insert_from_artifact(qdb.artifact.Artifact(aid), {})
        self.assertEqual(str(err.exception), "The artifact has no biom files")
        ...
        obs = qdb.archive.Archive.retrieve_feature_values("Single Rarefaction | N/A")
        ...
        self.assertDictEqual(
            qdb.archive.Archive.merging_schemes(),
            {
                1: "Pick closed-reference OTUs | Split libraries FASTQ",
                2: "",
                3: "Single Rarefaction | N/A",
            },
        )
