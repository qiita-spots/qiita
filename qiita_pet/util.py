...
STATUS_STYLER = {
    "sandbox": ("glyphicon glyphicon-eye-close", "glyphicon glyphicon-lock", "gray"),
    "awaiting_approval": (
        "glyphicon glyphicon-eye-open",
        "glyphicon glyphicon-lock",
        "peru",
    ),
    "private": ("glyphicon glyphicon-eye-open", "glyphicon glyphicon-lock", "#3599FD"),
    "public": ("glyphicon glyphicon-eye-open", "glyphicon glyphicon-globe", "green"),
}

EBI_LINKIFIER = (
    '<a href="http://www.ebi.ac.uk/ena/data/view/{0}" target="_blank">{0}</a>'
)
    ...
    for job in artifact.jobs():
        ...
        if job_status == "error":
            ...
            preprocessing_status_msg.append(
                "<b>Job %s</b>: failed - %s" % (job.id, job.log.msg)
            )
            ...
            preprocessing_status_msg.append("<b>Job %s</b>: %s" % (job.id, job_status))
