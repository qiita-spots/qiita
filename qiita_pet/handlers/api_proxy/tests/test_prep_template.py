...
class TestPrepAPIReadOnly(TestCase):
    ...
    def test_check_prep_template_exists_no_template(self):
        ...
        self.assertEqual(
            obs, {"status": "error", "message": "Prep template 3100 does not exist"}
        )

    def test_prep_template_get_req(self):
        obs = prep_template_get_req(1, "test@foo.bar")
        self.assertCountEqual(list(obs.keys()), ["status", "message", "template"])
        ...
        exp = {
            ...
            "summary": [
                ...
                (
                    ...
                    [
                        (
                            "micro biome of soil and "
                            "rhizosphere of cannabis plants from CA",
                            27,
                        )
                    ],
                ),
...
class TestPrepAPI(TestCase):
    def setUp(self):
        ...
        self.update_fp = join(get_mountpoint("uploads")[0][1], "1", "update.txt")
    ...
    def test_prep_template_jobs_get_req(self):
        ...
        prep_template_patch_req(
            "test@foo.bar", "remove", "/%s/10/columns/target_subfragment/" % pt.id
        )
    ...
    def test_prep_template_post_req(self):
        obs = prep_template_post_req(1, "test@foo.bar", "update.txt", "16S", name="  ")
        exp = {
            "status": "warning",
            "message": [
                "Both a converter and dtype were specified for column "
                "sample_name - only the converter will be used.",
    ...
    def test_prep_template_post_req_errors(self):
        ...
        obs = prep_template_post_req(3100, "test@foo.bar", "update.txt", "16S")
        self.assertEqual(obs, {"status": "error", "message": "Study does not exist"})
    ...
    def test_prep_template_patch_req(self):
        ...
        obs = prep_template_patch_req(
            "test@foo.bar", "replace", "/%s/investigation_type" % pt.id, "RNA-Seq"
        )
        ...
        obs = prep_template_patch_req(
            "test@foo.bar", "add", "/1/investigation_type", "RNA-Seq"
        )
        exp = {
            "status": "error",
        ...
        obs = prep_template_patch_req(
            "test@foo.bar", "replace", "/investigation_type", "RNA-Seq"
        )
        exp = {"status": "error", "message": "Incorrect path parameter"}
        self.assertEqual(obs, exp)
        # Incorrect attribute
        obs = prep_template_patch_req(
            "test@foo.bar", "replace", "/1/other_attribute", "RNA-Seq"
        )
        exp = {
            "status": "error",
        ...
        # User doesn't have access
        obs = prep_template_patch_req(
            "demo@microbio.me", "replace", "/%s/investigation_type" % pt.id, "RNA-Seq"
        )
        exp = {"status": "error", "message": "User has insufficient permissions"}
        self.assertEqual(obs, exp)
    ...
    def test_prep_template_delete_req(self):
        ...
        npt.assert_warns(QiitaDBWarning, PrepTemplate.create, template, Study(1), "16S")
    ...
    def test_prep_template_delete_req_no_prep(self):
        ...
        exp = {"status": "error", "message": "Prep template 3100 does not exist"}
        self.assertEqual(obs, exp)
