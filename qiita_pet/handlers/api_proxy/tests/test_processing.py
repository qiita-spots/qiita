...
class TestProcessingAPI(TestCase):
    def test_workflow_handler_post_req(self):
        ...
        self.assertRegex(
            obs.pop("message"),
            "Cannot create job because the parameters are "
    ...
    def test_workflow_handler_patch_req(self):
        ...
        obs = workflow_handler_patch_req("add", "/%s/" % wf.id, req_value=dumps(value))
        ...
        obs = workflow_handler_patch_req("remove", "/%s/%s/" % (wf.id, new_job.id))
    ...
    def test_workflow_handler_patch_req_error(self):
        ...
        exp = {"status": "error", "message": "Workflow 1000 does not exist"}
        self.assertEqual(obs, exp)
    ...
    def test_job_ajax_patch_req(self):
        ...
        exp = {"status": "error", "message": "Incorrect path parameter: missing job id"}
        self.assertEqual(obs, exp)
        ...
        exp = {
            "status": "error",
            "message": "Incorrect path parameter: notAJobId is not a recognized job id",
        }
        self.assertEqual(obs, exp)

        # Job doesn't exist
        obs = job_ajax_patch_req("remove", "/6d368e16-2242-4cf8-87b4-a5dc40bc890b")
        exp = {
            "status": "error",
