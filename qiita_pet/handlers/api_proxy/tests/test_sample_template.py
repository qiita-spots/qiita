...
class TestSampleAPI(TestCase):
    def setUp(self):
        ...
        base_dir = join(
            qdb.util.get_mountpoint("uploads")[0][1], str(self.new_study.id)
        )
    ...
    def test_check_sample_template_exists_no_template(self):
        ...
        self.assertEqual(
            ...
            {
                "status": "error",
                "message": "Sample template %d does not exist" % self.new_study.id,
            },
        )
    ...
    def test_analyses_associated_with_study(self):
        ...
        obs = analyses_associated_with_study(self.new_study.id, "shared@foo.bar")
        exp = {"status": "error", "message": "User has insufficient permissions"}
        self.assertEqual(obs, exp)
    ...
    def test_sample_template_sample_get_req_no_template(self):
        obs = sample_template_samples_get_req(self.new_study.id, "test@foo.bar")
        self.assertEqual(
            obs,
            {
                "status": "error",
    ...
    def test_sample_template_category_get_req_no_access(self):
        obs = sample_template_category_get_req("latitude", 1, "demo@microbio.me")
        exp = {"status": "error", "message": "User has insufficient permissions"}
        self.assertEqual(obs, exp)

    def test_sample_template_category_get_req_no_template(self):
        obs = sample_template_category_get_req(
            "latitiude", self.new_study.id, "test@foo.bar"
        )
        self.assertEqual(
            obs,
            {
    ...
    def test_sample_template_filepaths_get_req_no_template(self):
        obs = sample_template_filepaths_get_req(self.new_study.id, "test@foo.bar")
        self.assertEqual(
            obs,
            {
                "status": "error",
    ...
    def test_sample_template_meta_cats_get_req_no_template(self):
        obs = sample_template_meta_cats_get_req(self.new_study.id, "test@foo.bar")
        self.assertEqual(
            obs,
            {
                "status": "error",
