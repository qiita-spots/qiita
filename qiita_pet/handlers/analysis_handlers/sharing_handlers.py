...
class ShareAnalysisAJAX(BaseHandler):
    ...
    def _share(self, analysis, user, callback):
        user = User(user)
        add_message(
            "Analysis <a href=\"%s/analysis/description/%d/\">'%s'</a> "
    ...
    def _unshare(self, analysis, user, callback):
        user = User(user)
        add_message("Analysis '%s' has been unshared with you." % analysis.name, [user])
    ...
    def get(self):
        ...
        if not analysis.has_access(self.current_user):
            raise HTTPError(
                403,
                reason="User %s does not have permissions to "
                "share analysis %s" % (self.current_user.id, analysis.id),
            )
