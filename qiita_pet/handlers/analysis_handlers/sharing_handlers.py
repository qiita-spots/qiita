...
class ShareAnalysisAJAX(BaseHandler):
    ...
    def _share(self, analysis, user, callback):
        ...
        add_message(
            ...
            'has been shared with you.' % (
                qiita_config.portal_dir, analysis.id, analysis.name), [user])
    ...
    def get(self):
        ...
        if not analysis.has_access(self.current_user):
            raise HTTPError(403, reason='User %s does not have permissions to '
                            'share analysis %s' % (self.current_user.id,
                                                   analysis.id))
