    ...
    def get(self, requested_filepath):
        ...
        if not filepath.startswith(basedatadir):
            ...
            raise HTTPError(
                403,
                reason=(
                    "You cannot access files outside of the BASE_DATA_DIR of Qiita!"
                ),
            )
        ...
        if not os.path.exists(filepath):
            raise HTTPError(
                403,
                reason=("The requested file is not present in Qiita's BASE_DATA_DIR!"),
            )
...
class PushFileToCentralHandler(RequestHandler):
    ...
    def post(self):
        ...
        self.write(
            "Stored %i files into BASE_DATA_DIR of Qiita:\n%s\n"
