...
class FetchFileFromCentralHandlerTests(OauthTestingBase):
    ...
    def test_get(self):
        ...
        obs = self.get_authed(endpoint + base_data_dir[1:] + "/nonexistingfile")
        self.assertEqual(obs.status_code, 403)
        self.assertIn("The requested file is not present", obs.reason)

        obs = self.get_authed(
            endpoint + base_data_dir[1:] + "/raw_data/FASTA_QUAL_preprocessing.fna"
        )
...
class PushFileToCentralHandlerTests(OauthTestingBase):
    ...
    def test_post(self):
        ...
        with open(fp_source, "rb") as fh:
            ...
            self.assertIn(
                "Stored 1 files into BASE_DATA_DIR of Qiita", str(obs.content)
            )
        ...
        with open(fp_source, "rb") as fh:
            obs = self.post_authed(endpoint, files={"bar/": fh})
            self.assertIn("already present in Qiita's BASE_DATA_DIR!", obs.reason)
        ...
        with open(fp_source, "rb") as fh1:
            with open(fp_source2, "rb") as fh2:
                obs = self.post_authed(endpoint, files={"bar/": fh1, "barr/": fh2})
                self.assertIn(
                    "Stored 2 files into BASE_DATA_DIR of Qiita", str(obs.content)
                )
                self.assertTrue(filecmp.cmp(fp_source, fp_target, shallow=False))
                self.assertTrue(filecmp.cmp(fp_source2, fp_target2, shallow=False))
