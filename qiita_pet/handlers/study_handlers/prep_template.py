...
class NewPrepTemplateAjax(BaseHandler):
    ...
    def get(self):
        ...
        self.render(
            "study_ajax/add_prep_template.html",
...
class AddDefaultWorkflowHandler(BaseHandler):
    ...
    def post(self):
        ...
        data = None
        try:
            workflow = PrepTemplate(prep_id).add_default_workflow(self.current_user)
            data = workflow.id
...
class PrepTemplateSummaryAJAX(BaseHandler):
    ...
    def get(self):
        ...
        self.render(
            "study_ajax/prep_summary_table.html",
            pid=prep_id,
            stats=res["summary"],
            ...
            if "sample_sheet" in params:
                ...
                if res["creation_job"].status == "success":
                    if res["creation_job"].outputs:
                        ...
                        _file = res["creation_job"].outputs["output"].html_summary_fp[1]
