...
class ArtifactGraphAJAX(BaseHandler):
    ...
    def get(self):
        ...
        self.write(artifact_graph_get_req(artifact, direction, self.current_user.id))
...
class NewArtifactHandler(BaseHandler):
    ...
    def get(self):
        ...
        artifact_types = [
            (at, desc)
            for at, desc, _, _, is_user_uploadable in artifact_types_get_req()["types"]
            if is_user_uploadable
        ]

        self.render(
            "study_ajax/add_artifact.html",
            study_id=study_id,
            prep_id=prep_id,
    ...
    def post(self):
        ...
        artifact = artifact_post_req(
            self.current_user.id, files, artifact_type, name, prep_id, artifact_id
        )
        self.write(artifact)
...
class ArtifactAdminAJAX(BaseHandler):
    ...
    def get(self):
        ...
        btn_base = (
            "<button onclick=\"set_admin_visibility('%s', {0})\" "
        ...
        if qiita_config.require_approval:
            if status == "sandbox":
                ...
                buttons.append(btn_base % ("awaiting_approval", "Request approval"))
            elif self.current_user.level == "admin" and status == "awaiting_approval":
        ...
        if not info["is_submitted_vamps"] and info["can_submit_vamps"]:
            buttons.append(
                '<a class="btn btn-primary glyphicon '
    ...
    def post(self):
        ...
        response = artifact_status_put_req(
            artifact_id, self.current_user.id, visibility
        )
        self.write(response)
