...
class ArtifactGraphAJAXTests(TestHandlerBase):
    def test_get_ancestors(self):
        response = self.get(
            "/artifact/graph/", {"direction": "ancestors", "artifact_id": 1}
        )
        exp = {
            "status": "success",
            "message": "",
    ...
    def test_get_descendants(self):
        response = self.get(
            "/artifact/graph/", {"direction": "descendants", "artifact_id": 1}
        )
        exp = {
            "status": "success",
            "message": "",
    ...
    def test_get_unknown(self):
        response = self.get("/artifact/graph/", {"direction": "BAD", "artifact_id": 1})
        exp = {"status": "error", "message": "Unknown directon BAD"}
        self.assertEqual(response.code, 200)
        self.assertEqual(loads(response.body), exp)
...
class NewArtifactHandlerTests(TestHandlerBase):
    ...
    def test_post_artifact(self):
        ...
        self.assertEqual(response.code, 200)
        self.assertEqual(loads(response.body), {"status": "success", "message": ""})
...
class ArtifactAdminAJAXTestsReadOnly(TestHandlerBase):
    def test_get_admin(self):
        response = self.get("/admin/artifact/", {"artifact_id": 3})
        self.assertEqual(response.code, 200)
        body = response.body.decode("ascii")

        # checking that proper actions shown
        self.assertIn("Make public</button>", body)
        self.assertIn("Revert to sandbox</button>", body)
        self.assertIn("Submit to EBI</a>", body)
        self.assertIn("Submit to VAMPS</a>", body)


class ArtifactAdminAJAXTests(TestHandlerBase):
    def test_post_admin(self):
        response = self.post(
            "/admin/artifact/", {"artifact_id": 3, "visibility": "sandbox"}
        )
        self.assertEqual(response.code, 200)

        # checking that proper actions shown
        self.assertEqual(
            {"status": "success", "message": "Artifact visibility changed to sandbox"},
            loads(response.body),
        )
