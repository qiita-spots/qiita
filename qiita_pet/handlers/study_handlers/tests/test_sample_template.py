...
class TestHelpers(TestHandlerBase):
    ...
    def test_sample_template_checks(self):
        ...
        with self.assertRaisesRegex(HTTPError, "User has insufficient permissions"):
        ...
        with self.assertRaisesRegex(
            HTTPError, "Study %s doesn't have sample information" % new_study.id
        ):
    ...
    def test_sample_template_handler_post_request(self):
        ...
        with self.assertRaisesRegex(HTTPError, "User has insufficient permissions"):
            sample_template_handler_post_request(1, User("demo@microbio.me"), "ignored")
        ...
        with self.assertRaisesRegex(
            HTTPError, "Please, choose a data type if uploading a QIIME mapping file"
        ):
            sample_template_handler_post_request(1, user, fp)

        # Test success
        obs = sample_template_handler_post_request(1, user, "uploaded_file.txt")
    ...
    def test_sample_template_handler_patch_request(self):
        ...
        with self.assertRaisesRegex(HTTPError, "User has insufficient permissions"):
            sample_template_handler_patch_request(
                User("demo@microbio.me"), "remove", "/1/columns/season_environment/"
            )
        ...
        with self.assertRaisesRegex(
            ...
            sample_template_handler_patch_request(
                user, "remove", "/%s/columns/season_environment/" % new_study.id
            )

        # Test wrong operation value
        with self.assertRaisesRegex(
            HTTPError,
            "Operation add not supported. Current supported operations: remove.",
        ):
            sample_template_handler_patch_request(
        ...
        md = pd.DataFrame.from_dict(
            {"Sample1": {"col1": "val1", "col2": "val2"}}, orient="index", dtype=str
        )
        ...
        # Test missing value
        with self.assertRaisesRegex(
            HTTPError, "Value is required when updating sample information"
        ):
        ...
        with self.assertRaisesRegex(HTTPError, "Filepath not found"):
            sample_template_handler_patch_request(
                user, "replace", "/1/data", req_value="DoesNotExist"
            )
    ...
    def test_sample_template_handler_delete_request(self):
        # Test user doesn't have access
        with self.assertRaisesRegex(HTTPError, "User has insufficient permissions"):
            sample_template_handler_delete_request(1, User("demo@microbio.me"))
    ...
    def test_sample_template_overview_handler_get_request(self):
        # Test user doesn't have access
        with self.assertRaisesRegex(HTTPError, "User has insufficient permissions"):
            sample_template_overview_handler_get_request(1, User("demo@microbio.me"))
    ...
    def test_build_sample_summary(self):
        ...
        rows_exp = {
            "1.SKB2.640194": {"sample": "1.SKB2.640194", "prep2": "X", "prep1": "X"},
            "1.SKM4.640180": {"sample": "1.SKM4.640180", "prep2": "X", "prep1": "X"},
            "1.SKB3.640195": {"sample": "1.SKB3.640195", "prep2": "X", "prep1": "X"},
            "1.SKB6.640176": {"sample": "1.SKB6.640176", "prep2": "X", "prep1": "X"},
            "1.SKD6.640190": {"sample": "1.SKD6.640190", "prep2": "X", "prep1": "X"},
            "1.SKM6.640187": {"sample": "1.SKM6.640187", "prep2": "X", "prep1": "X"},
            "1.SKD9.640182": {"sample": "1.SKD9.640182", "prep2": "X", "prep1": "X"},
            "1.SKM8.640201": {"sample": "1.SKM8.640201", "prep2": "X", "prep1": "X"},
            "1.SKM2.640199": {"sample": "1.SKM2.640199", "prep2": "X", "prep1": "X"},
            "1.SKD2.640178": {"sample": "1.SKD2.640178", "prep2": "X", "prep1": "X"},
            "1.SKB7.640196": {"sample": "1.SKB7.640196", "prep2": "X", "prep1": "X"},
            "1.SKD4.640185": {"sample": "1.SKD4.640185", "prep2": "X", "prep1": "X"},
            "1.SKB8.640193": {"sample": "1.SKB8.640193", "prep2": "X", "prep1": "X"},
            "1.SKM3.640197": {"sample": "1.SKM3.640197", "prep2": "X", "prep1": "X"},
            "1.SKD5.640186": {"sample": "1.SKD5.640186", "prep2": "X", "prep1": "X"},
            "1.SKB1.640202": {"sample": "1.SKB1.640202", "prep2": "X", "prep1": "X"},
            "1.SKM1.640183": {"sample": "1.SKM1.640183", "prep2": "X", "prep1": "X"},
            "1.SKD1.640179": {"sample": "1.SKD1.640179", "prep2": "X", "prep1": "X"},
            "1.SKD3.640198": {"sample": "1.SKD3.640198", "prep2": "X", "prep1": "X"},
            "1.SKB5.640181": {"sample": "1.SKB5.640181", "prep2": "X", "prep1": "X"},
            "1.SKB4.640189": {"sample": "1.SKB4.640189", "prep2": "X", "prep1": "X"},
            "1.SKB9.640200": {"sample": "1.SKB9.640200", "prep2": "X", "prep1": "X"},
            "1.SKM9.640192": {"sample": "1.SKM9.640192", "prep2": "X", "prep1": "X"},
            "1.SKD8.640184": {"sample": "1.SKD8.640184", "prep2": "X", "prep1": "X"},
            "1.SKM5.640177": {"sample": "1.SKM5.640177", "prep2": "X", "prep1": "X"},
            "1.SKM7.640188": {"sample": "1.SKM7.640188", "prep2": "X", "prep1": "X"},
            "1.SKD7.640191": {"sample": "1.SKD7.640191", "prep2": "X", "prep1": "X"},
        }
...
class TestSampleTemplateHandler(TestHandlerBase):
    def test_get(self):
        response = self.get("/study/description/sample_template/", {"study_id": 1})
        self.assertEqual(response.code, 200)
        self.assertNotEqual(response.body, "")

        # Study doesn't exist
        response = self.get("/study/description/sample_template/", {"study_id": 10000})
    ...
    def test_post(self):
        response = self.post(
            ...
            {"study_id": 1, "filepath": "uploaded_file.txt", "data_type": ""},
        )
        self.assertEqual(response.code, 200)
    ...
    def test_patch(self):
        response = self.patch(
            "/study/description/sample_template/",
            {"op": "replace", "path": "/1/data", "value": "uploaded_file.txt"},
        )
        self.assertEqual(response.code, 200)
        self.assertIsNotNone(response.body)
        obs = loads(response.body)
    ...
    def test_delete(self):
        response = self.delete("/study/description/sample_template/", {"study_id": 1})
        self.assertEqual(response.code, 200)
        self.assertIsNotNone(response.body)
        obs = loads(response.body)
        self.assertCountEqual(obs.keys(), ["job"])
...
class TestSampleTemplateOverviewHandler(TestHandlerBase):
    def test_get(self):
        response = self.get(
            "/study/description/sample_template/overview/", {"study_id": 1}
        )
        self.assertEqual(response.code, 200)
        self.assertIsNotNone(response.body)
        obs = loads(response.body)
...
class TestSampleTemplateColumnsHandler(TestHandlerBase):
    def test_get(self):
        response = self.get(
            "/study/description/sample_template/columns/", {"study_id": 1}
        )
        self.assertEqual(response.code, 200)
        self.assertIsNotNone(response.body)
        obs = loads(response.body)
...
class TestSampleAJAX(TestHandlerBase):
    def test_post(self):
        res = self.post(
            "/study/description/sample_summary/",
    ...
    def test_post_error(self):
        res = self.post(
            "/study/description/sample_summary/", {"study_id": 1, "meta_col": "NOEXIST"}
        )
        self.assertEqual(res.code, 200)
        exp = {
            "status": "error",
