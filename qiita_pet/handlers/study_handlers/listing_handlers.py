...
class StudyApprovalList(BaseHandler):
    ...
    def get(self):
        ...
        if user.level != "admin":
            raise HTTPError(403, reason="User %s is not admin" % self.current_user)

        studies = defaultdict(list)
        for artifact in Artifact.iter_by_visibility("awaiting_approval"):
            studies[artifact.study].append(artifact.id)
        parsed_studies = [
            (s.id, s.title, s.owner.email, pds) for s, pds in studies.items()
        ]
...
class AutocompleteHandler(BaseHandler):
    ...
    def get(self):
        ...
        vals = r_client.execute_command(
            "zrangebylex", "qiita-usernames", "[%s" % text, "[%s\xff" % text
        )
        self.write(
            {
                "results": [
                    {"id": s.decode("utf-8"), "text": s.decode("utf-8")} for s in vals
                ]
            }
        )
...
class ShareStudyAJAX(BaseHandler):
    ...
    def _share(self, study, user, callback):
        ...
        add_message(
            ...
            % (qiita_config.portal_dir, study.id, study.title),
            [user],
        )
    ...
    def _unshare(self, study, user, callback):
        user = User(user)
        add_message("Study '%s' has been unshared from you." % study.title, [user])
...
class ListStudiesAJAX(BaseHandler):
    ...
    def get(self, ignore):
        ...
        for i in range(len_info):
            info[i]["shared"] = ", ".join(
                [study_person_linkifier(element) for element in info[i]["shared"]]
            )
