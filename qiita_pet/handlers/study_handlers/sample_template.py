...
def sample_template_checks(study_id, user, check_exists=False, no_public=False):
...
def sample_template_handler_post_request(
    study_id, user, filepath, data_type=None, direct_upload=False
):
    ...
    if is_mapping_file and not data_type:
        raise HTTPError(
            400, reason="Please, choose a data type if uploading a QIIME mapping file"
        )
    ...
    r_client.set(SAMPLE_TEMPLATE_KEY_FORMAT % study_id, dumps({"job_id": job.id}))
    job.submit()
    return {"job": job.id}


def sample_template_handler_patch_request(
    user, req_op, req_path, req_value=None, req_from=None, direct_upload=False
):
    ...
    if req_op == "remove":
        ...
        if attribute == "data":
            ...
            if req_value is None:
                raise HTTPError(
                    400, reason="Value is required when updating sample information"
                )
            ...
            params = Parameters.load(
                cmd, values_dict={"study": study_id, "template_fp": filepath}
            )
            job = ProcessingJob.create(user, params, True)

            # Store the job id attaching it to the sample template id
            r_client.set(
                SAMPLE_TEMPLATE_KEY_FORMAT % study_id, dumps({"job_id": job.id})
            )
...
class SampleTemplateHandler(BaseHandler):
    ...
    def patch(self):
        ...
        self.write(
            sample_template_handler_patch_request(
                self.current_user, req_op, req_path, req_value, req_from, direct_upload
            )
        )
    ...
    def delete(self):
        study_id = int(self.get_argument("study_id"))
        self.write(sample_template_handler_delete_request(study_id, self.current_user))
...
class SampleTemplateOverviewHandler(BaseHandler):
    ...
    def get(self):
        study_id = int(self.get_argument("study_id"))
        self.write(
            sample_template_overview_handler_get_request(study_id, self.current_user)
        )
...
def _build_sample_summary(study_id, user_id):
    ...
    for preptype in preps:
        for prep in preps[preptype]:
            ...
            columns[field] = name
            prep_samples = prep_template_samples_get_req(prep["id"], user_id)["samples"]
            for s in samples:
...
class SampleAJAX(BaseHandler):
    ...
    def get(self):
        ...
        _, alert_type, alert_msg = get_sample_template_processing_status(study_id)

        self.render(
            "study_ajax/sample_prep_summary.html",
            rows=rows,
            columns=columns,
    ...
    def post(self):
        ...
        values = sample_template_category_get_req(
            meta_col, study_id, self.current_user.id
        )
        if values["status"] != "success":
            self.write(values)
        else:
            self.write({"status": "success", "message": "", "values": values["values"]})
...
class AnalysesAjax(BaseHandler):
    ...
    def get(self):
        ...
        self.render("study_ajax/study_analyses.html", analyses=result["values"])
