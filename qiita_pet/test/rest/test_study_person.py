...
class StudyPersonHandlerTests(RESTHandlerTestCase):
    ...
    def test_exist(self):
        ...
        response = self.get(
            "/api/v1/person?name=LabDude&affiliation=knight%20lab", headers=self.headers
        )
        self.assertEqual(response.code, 200)
        obs = json_decode(response.body)
    ...
    def test_get_does_not_exist(self):
        ...
        response = self.get(
            "/api/v1/person?name=Boaty%20McBoatFace&affiliation=UCSD",
    ...
    def test_get_does_not_exist_affiliation(self):
        exp = {"message": "Person not found"}
        response = self.get(
            "/api/v1/person?name=LabDude%20&affiliation=UCSD", headers=self.headers
        )
        self.assertEqual(response.code, 404)
        obs = json_decode(response.body)
        self.assertEqual(obs, exp)
    ...
    def test_get_invalid_query_string_2(self):
        response = self.get(
            "/api/v1/person?affiliation=knight%20lab", headers=self.headers
        )
        self.assertEqual(response.code, 400)
    ...
    def test_get_valid_extra_arguments(self):
        ...
        response = self.get(
            "/api/v1/person?name=LabDude&affiliation=knight%20lab&foo=bar",
            headers=self.headers,
        )
        self.assertEqual(response.code, 200)
        obs = json_decode(response.body)
    ...
    def test_post_new_person(self):
        ...
        obs = json_decode(response.body)
        exp = StudyPerson.from_name_and_affiliation(
            body["name"], body["affiliation"]
        ).id
