...
class StudyHandlerTests(RESTHandlerTestCase):
    def test_get_valid(self):
        exp = {
            ...
            "contacts": {
                "principal_investigator": ["PIDude", "Wash U", "PI_dude@foo.bar"],
                "lab_person": ["LabDude", "knight lab", "lab_dude@foo.bar"],
            },
            "study_abstract": (
    ...
    def test_get_invalid(self):
        ...
        self.assertEqual(json_decode(response.body), {"message": "Study not found"})
...
class StudyCreatorTests(RESTHandlerTestCase):
    def test_post_malformed_study(self):
        response = self.post(
            "/api/v1/study", data={"foo": "bar"}, headers=self.headers, asjson=True
        )
    ...
    def test_post_already_exists(self):
        payload = {
            ...
            "notes": "",
            "contacts": {
                "principal_investigator": ["PIDude", "PI_dude@foo.bar"],
                "lab_person": ["LabDude", "lab_dude@foo.bar"],
            },
        }
        response = self.post(
            "/api/v1/study", data=payload, asjson=True, headers=self.headers
        )
        self.assertEqual(response.code, 409)
        obs = json_decode(response.body)
        self.assertEqual(obs, {"message": "Study title already exists"})
    ...
    def test_post_valid(self):
        ...
        response = self.post(
            "/api/v1/study", data=payload, headers=self.headers, asjson=True
        )
        ...
        self.assertEqual(study.info["study_abstract"], payload["study_abstract"])
        self.assertEqual(study.info["study_description"], payload["study_description"])
        self.assertEqual(study.info["study_alias"], payload["study_alias"])
        self.assertEqual(study.owner.email, payload["owner"])
        self.assertEqual(
            study.info["principal_investigator"].name,
            payload["contacts"]["principal_investigator"][0],
        )
        self.assertEqual(
            study.info["principal_investigator"].affiliation,
            payload["contacts"]["principal_investigator"][1],
        )
        self.assertEqual(
            study.info["lab_person"].name, payload["contacts"]["lab_person"][0]
        )
        self.assertEqual(
            study.info["lab_person"].affiliation, payload["contacts"]["lab_person"][1]
        )
