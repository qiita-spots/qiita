...
class TestPortal(TestHandlerBase):
    ...
    def test_post_add(self):
        BaseHandler.get_current_user = Mock(return_value=User("admin@foo.bar"))
        response = self.post(
            "/admin/portals/studies/",
            {"portal": "EMP", "selected": [1], "action": "Add"},
        )
        self.assertEqual(response.code, 200)
        self.assertNotEqual(response.body, "")
    ...
    def test_post_remove(self):
        ...
        response = self.post(
            "/admin/portals/studies/",
            {"portal": "EMP", "selected": [1], "action": "Remove"},
        )
        self.assertEqual(response.code, 200)
        self.assertNotEqual(response.body, "")
    ...
    def test_post_not_valid_portal(self):
        ...
        response = self.post(
            "/admin/portals/studies/",
            {"portal": "not-valid", "selected": [1], "action": "Add"},
        )
    ...
    def test_post_not_valid_action(self):
        ...
        response = self.post(
            "/admin/portals/studies/",
            {"portal": "EMP", "selected": [1], "action": "Error"},
        )
        self.assertEqual(response.code, 400)
