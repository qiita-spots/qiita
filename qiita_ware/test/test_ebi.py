...
class TestEBISubmission(TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(
            e.study_title, "Identification of the Microbiomes for Cannabis Soils"
        )
        self.assertEqual(
            e.study_abstract,
            (
    ...
    def test_get_submission_alias(self):
        ...
        exp = "%s_submission_%d" % (qiita_config.ebi_organization_prefix, artifact_id)
    ...
    def test_get_run_alias(self):
        ...
        e = EBISubmission(artifact_id, "ADD")
        self.files_to_remove.append(e.full_ebi_dir)
        exp = "%s_ppdid_%d:foo" % (qiita_config.ebi_organization_prefix, artifact_id)
    ...
    def test_add_dict_as_tags_and_values(self):
        ...
        e._add_dict_as_tags_and_values(elm, "foo", {"x": "y", ">x": "<y", "none": None})
        obs = ET.tostring(elm)
    ...
    def test_generate_sample_xml(self):
        ...
        exp = (
            '<SAMPLE_SET xmlns:xsi="http://www.w3.org/2001/XMLSchema-'
    ...
    def test_generate_submission_xml(self):
        ...
        obs = ET.tostring(
            submission.generate_submission_xml(submission_date=date(2015, 9, 3))
        )
        exp = SUBMISSIONXML % {
    ...
    def test_generate_curl_command(self):
        ...
        obs = submission.generate_curl_command(
            test_ebi_seq_xfer_user, test_ebi_seq_xfer_pass, test_ebi_dropbox_url
        )
        exp = (
            "curl -sS -k "
    ...
    def write_demux_files(self, prep_template, sequences="FASTA-EXAMPLE"):
        ...
        if sequences == "FASTA-EXAMPLE":
            ...
            raise ValueError(
                "Wrong sequences values: %s. Valid values: "
    ...
    def generate_new_prep_template_and_write_demux_files(self, valid_metadata=False):
        ...
        if not valid_metadata:
            metadata_dict = {
                ...
                "SKD9.640182": {
                    ...
                    "experiment_design_description": "microbiome of soil and rhizosphere",
                    "library_construction_protocol": "PMID: 22402401",
                },
            }
        ...
        with warnings.catch_warnings(record=True):
            pt = PrepTemplate.create(
                metadata, Study(1), "18S", investigation_type=investigation_type
            )
    ...
    def generate_new_study_with_preprocessed_data(self):
        ...
        metadata_dict = {
            ...
            "Sample2": {
                "collection_timestamp": "06/02/15 07:00:00",
                "physical_specimen_location": "location1",
                "taxon_id": 9606,
                "scientific_name": "homo sapiens",
            ...
            "Sample3": {
                "collection_timestamp": "06/03/15 07:00:00",
                "physical_specimen_location": "location1",
                "taxon_id": 9606,
                "scientific_name": "homo sapiens",
        ...
        artifact = Artifact.create([(demux_fp, 6)], "Demultiplexed", prep_template=pt)
    ...
    def test_init_exceptions(self):
        ...
        exp_text = (
            "Errors found during EBI submission for study #1, "
    ...
    def test_generate_demultiplexed_fastq(self):
        ...
        self.assertCountEqual(obs_demux_samples, ebi_submission.samples_prep.keys())
    ...
    def test_generate_demultiplexed_per_sample_fastq(self):
        ...
        artifact = self._generate_per_sample_FASTQs(PrepTemplate(1), FASTQ_EXAMPLE)
        ebi_submission = EBISubmission(artifact.id, "ADD")
        self.files_to_remove.append(ebi_submission.full_ebi_dir)
        ...
        with warnings.catch_warnings(record=True):
            pt = PrepTemplate.create(
                metadata, Study(1), "18S", investigation_type="Metagenomics"
            )
        ...
        with self.assertRaises(EBISubmissionError) as err:
            ebi_submission = EBISubmission(artifact.id, "ADD")
        self.assertIn(
            "Missing column in the prep template: barcode", str(err.exception)
        )
        metadata_dict = {
    ...
    def test_generate_demultiplexed_per_sample_fastq_reverse(self):
        ...
        for fn, _ in filepaths:
            with open(fn, "w") as fh:
                fh.write("some text")
            self.files_to_remove.append(fn)
        artifact = Artifact.create(filepaths, "per_sample_FASTQ", prep_template=pt)
    ...
    def test_generate_send_sequences_cmd(self):
        ...
        exp = (
            ...
            "Webin-41528@webin.ebi.ac.uk:./%(aid)d_ebi_submission/"
            % {"ebi_dir": e.full_ebi_dir, "aid": artifact.id}
        ).split("\n")
    ...
    def test_parse_EBI_reply(self):
        ...
        curl_result = (
            'some general text success="true" more text'
        ...
        curl_result = CURL_RESULT.format(
            qiita_config.ebi_organization_prefix, artifact.id
        )
        stacc, saacc, bioacc, exacc, runacc = e.parse_EBI_reply(curl_result)
