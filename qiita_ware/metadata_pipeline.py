...
def create_templates_from_qiime_mapping_file(fp, study, data_type):
    ...
    qiime_map.columns = [
        c.lower() if c.lower() in CONTROLLED_COLS else c for c in qiime_map.columns
    ]
    ...
    data_type_str = (
        convert_from_id(data_type, "data_type")
        if isinstance(data_type, int)
        else data_type
    )

    if data_type_str in TARGET_GENE_DATA_TYPES:
        pt_cols.update(col for col in _col_iterator(PREP_TEMPLATE_COLUMNS_TARGET_GENE))
    ...
    return (
        SampleTemplate.create(st_md, study),
